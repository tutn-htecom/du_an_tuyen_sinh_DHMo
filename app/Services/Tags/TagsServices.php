...
class TagsServices implements TagsInterface
{
    
    protected $tags_repository;
    public function __construct(TagsRepository $tags_repository)
    {
        $this->tags_repository = $tags_repository;   
    }
    public function index($params) {
        try {
            $model = $this->tags_repository;
    ...
    public function details($id) {
       try {
         $model = $this->tags_repository->where('id', $id, '=')->first();
         if(isset($model->id)) {
            $result = [
                "code" => 200,                
                "data" => $model
    ...
    public function create($params) {
       try {
            ...
            $model = $this->tags_repository->create($data);
            $result = null;
            if(isset($model->id)) {
                $result = [
                    "code" => 200,
    ...
    public function createMultiple($params) {
        try {
            ...
            $model = $this->tags_repository->createMultiple($data);
            $result = null;
            if (count($model) > 0) {
                $result = [
                    "code" => 200,
    ...
    public function update($params, $id) {
        ...
        try {
            ...
            $model = $this->tags_repository->updateById($id, $params);
            $result = null;
            if(isset($model->id)) {
                $result = [
                    "code" => 200,
    ...
    public function delete($id) {
        try {
            ...
            $model = $this->tags_repository->updateById($id, $data); 
            $result = null;
            if($model) {
                $result = [
                    "code" => 200,
