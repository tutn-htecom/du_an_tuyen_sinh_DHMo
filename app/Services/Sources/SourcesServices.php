...
class SourcesServices implements SourcesInterface
{
    ...
    public function __construct(SourcesRepository $sources_repository)
    {
        $this->sources_repository = $sources_repository;   
    }
    public function index($params) {
        try {
            $model = $this->sources_repository;
    ...
    public function details($id) {
       try {
         $model = $this->sources_repository->where('id', $id, '=')->first();
         if(isset($model->id)) {
            $result = [
                "code" => 200,                
                "data" => $model
    ...
    public function createMultiple($params) {
        try {
            ...
            $model = $this->sources_repository->createMultiple($data);
            $result = null;
            if (count($model)) {
                $result = [
                    "code" => 200,
