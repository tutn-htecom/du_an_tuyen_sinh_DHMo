...
class MarjorsServices implements MarjorsInterface
{    
    
    protected $marjors_repository;
    public function __construct(MarjorsRepository $marjors_repository)
    {
       $this->marjors_repository = $marjors_repository;   
    }
    public function index($params) {
        try {
            $model = $this->marjors_repository;
    ...
    {
       try {
         $model = $this->marjors_repository->where('id', $id, '=')->first();
         if(isset($model->id)) {
            $result = [
                "code" => 200,                
                "data" => $model
    ...
    public function createMultiple($params){        
        try {    
            ...
            $model = $this->marjors_repository->createMultiple($data);
            $result = null;
            if(count($model) > 0) {
                $result = [
                    "code" => 200,
    ...
    public function create($params) {
       try {
            ...
            $model = $this->marjors_repository->create($data);
            $result = null;
            if(isset($model->id)) {
                $result = [
                    "code" => 200,
    ...
    public function update($params, $id) {
        try {
            ...
            $model = $this->marjors_repository->updateById($id, $params);
            $result = null;
            if(isset($model->id)) {
                $result = [
                    "code" => 200,
    ...
    public function delete($id) {
        try {
            ...
            $model = $this->marjors_repository->updateById($id, $data); 
            $result = null;
            if($model) {
                $result = [
                    "code" => 200,
